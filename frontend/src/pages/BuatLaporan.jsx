...
const BuatLaporan = () => {
  ...
  const handleDeleteCanceled = async (item) => {
    if (!window.confirm(`Apakah Anda yakin ingin menghapus pembatalan untuk SPT "${item.nomor_surat}"? Status SPT akan dikembalikan menjadi aktif.`)) return;
    try {
  ...
  const loadCancelPegawai = async (sptId, selectedPegawaiId = null) => {
    ...
    try {
      ...
      const available = (sptDetail.pegawai || []).filter(p => !signerSet.has(p.pegawai_id.toString()));

      setCancelPegawaiList(available);
      
      // Cache canceled IDs to show warning if selected
      const isAlreadyCanceled = canceledIds.includes(parseInt(selectedPegawaiId || cancelForm.pegawai_id, 10));
      if (isAlreadyCanceled && !cancelEditId) {
        setCancelNotif('Pegawai ini sudah pernah dibuatkan surat pembatalan tugas untuk SPT terkait.');
      }
    } catch (err) {
  ...
  const handleCancelSptChange = (e) => {
    ...
    setCancelForm(prev => ({ ...prev, spt_id: sptId, pegawai_id: '', ambilPanjar: false, rincian_biaya: '', nominal_biaya: '' }));
    loadCancelPegawai(sptId);
  };

  const handleCancelPegawaiChange = async (e) => {
    const pegawaiId = e.target.value;
    setCancelForm(prev => ({ ...prev, pegawai_id: pegawaiId, ambilPanjar: false, rincian_biaya: '', nominal_biaya: '' }));
    setCancelNotif('');
  ...
  const handlePanjarToggle = async (checked) => {
    ...
    if (checked) {
      ...
      try {
        const res = await axios.get(`/api/panjar/by-spt/${cancelForm.spt_id}/pegawai/${cancelForm.pegawai_id}`);
        if (res.data) {
          setCancelForm(prev => ({
            ...prev,
        ...
        console.error(err);
        alert('Info: ' + (err.response?.data?.message || 'Uang muka tidak ditemukan untuk pegawai ini.'));
        setCancelForm(prev => ({ ...prev, ambilPanjar: false }));
      }
    } else {
  ...
  const handleCancelSubmit = async (e) => {
    e.preventDefault();
    if (!cancelForm.spt_id || !cancelForm.pegawai_id || !cancelForm.tempat_pembatalan || !cancelForm.tanggal_pembatalan) {
    ...
    if (!cancelEditId) {
      const confirmText = 'Apakah Anda yakin ingin membatalkan SPT ini untuk pegawai pelaksana yang dipilih?';
      ...
      try {
        ...
        setSptOptions(prev => {
          if (!prev.some(opt => opt.id === laporan.spt_id)) {
            return [...prev, { id: laporan.spt_id, nomor_surat: laporan.nomor_surat || 'SPT Terkait' }];
          }
          return prev;
        });
        ...
        try {
          const standardsRes = await axios.get(`/api/spt/${laporan.spt_id}/accommodation-standards`);
        ...
        allPelaksana.forEach(p => {
          ...
          const tList = (laporan.transportasi || []).filter(t => t.pegawai_id == p.pegawai_id).map((t, index) => ({
            id: t.id || index,
            jenis: t.jenis || 'Bus',
          ...
          const aList = (laporan.akomodasi || []).filter(a => a.pegawai_id == p.pegawai_id).map((a, index) => ({
            id: a.id || index,
            jenis: a.jenis || 'Hotel',
            nama: a.nama || '',
          ...
          const kList = (laporan.kontribusi || []).filter(k => k.pegawai_id == p.pegawai_id).map((k, index) => ({
            id: k.id || index,
            jenis: k.jenis || 'Bimbingan Teknis',
            nominal: formatCurrency(k.nominal)
          }));

          const lList = (laporan.lain_lain || []).filter(l => l.pegawai_id == p.pegawai_id).map((l, index) => {
            let uName = l.uraian || '';
            try {
          ...
          initialPegawaiData[pegId] = {
            transportasi: tList.length > 0 ? tList : [{ jenis: 'Bus', perusahaan: '', kode_boking: '', nomor_penerbangan: '', nomor_tiket: '', tanggal_tiket: '', terminal_berangkat: '', terminal_tiba: '', nominal: '', arah_perjalanan: 'berangkat' }],
            akomodasi: aList.length > 0 ? aList : [{ jenis: 'Hotel', nama: '', lokasi_hotel: '', tanggal_checkIn: '', tanggal_checkOut: '', harga_satuan: '', malam: '', nominal: '', readOnlyHarga: false }],
            kontribusi: kList.length > 0 ? kList : [{ jenis: 'Bimbingan Teknis', nominal: '' }],
            lain_lain: lList.length > 0 ? lList : [{ uraian: '', tarif_satuan: '', jumlah_hari: '', nominal: '', keterangan: '' }]
          };
        });
  ...
  const handleSptChange = async (e) => {
    ...
    try {
      ...
      const canceledIds = new Set((sptDetail.pegawai_dibatalkan || []).map(p => p.pegawai_id.toString()));
      ...
      allPelaksana.forEach(p => {
        ...
        initData[pId] = {
          transportasi: [{ jenis: 'Bus', perusahaan: '', kode_boking: '', nomor_penerbangan: '', nomor_tiket: '', tanggal_tiket: '', terminal_berangkat: '', terminal_tiba: '', nominal: '', arah_perjalanan: 'berangkat' }],
          akomodasi: [{ jenis: 'Hotel', nama: '', lokasi_hotel: '', tanggal_checkIn: '', tanggal_checkOut: '', harga_satuan: '', malam: '', nominal: '', readOnlyHarga: false }],
          kontribusi: [{ jenis: 'Bimbingan Teknis', nominal: '' }],
          lain_lain: [{ uraian: '', tarif_satuan: '', jumlah_hari: '', nominal: '', keterangan: '' }]
        };
      });
  ...
  const addExpenseRow = (pegawaiId, category) => {
    setPegawaiData(prev => {
      ...
      if (category === 'transportasi') {
        newRow = { jenis: 'Bus', perusahaan: '', kode_boking: '', nomor_penerbangan: '', nomor_tiket: '', tanggal_tiket: '', terminal_berangkat: '', terminal_tiba: '', nominal: '', arah_perjalanan: 'berangkat' };
      } else if (category === 'akomodasi') {
        newRow = { jenis: 'Hotel', nama: '', lokasi_hotel: '', tanggal_checkIn: '', tanggal_checkOut: '', harga_satuan: '', malam: '', nominal: '', readOnlyHarga: false };
      ...
      if (category === 'akomodasi') {
        if (field === 'jenis') {
          if (value === 'Rumah Warga (30%)') {
            ...
            const calculatedCost = standard * 0.30;
    ...
    try {
      ...
      if (isEditMode) {
        try {
          ...
          if (paidCheck.data && paidCheck.data.exists) {
            alert('Aksi Diblokir: Laporan ini tidak dapat diubah karena sudah memiliki bukti bayar terkait. Hapus terlebih dahulu bukti bayar jika ingin mengedit laporan.');
            setSubmitting(false);
            return;
          }
        } catch (err) {
      ...
      Object.keys(pegawaiData).forEach(pegawaiId => {
        ...
        p.transportasi.forEach((item, index) => {
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][jenis]`, item.jenis || 'Bus');
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][perusahaan]`, item.perusahaan || '');
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][kode_boking]`, item.kode_boking || '');
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][nomor_penerbangan]`, item.nomor_penerbangan || '');
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][nomor_tiket]`, item.nomor_tiket || '');
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][tanggal_tiket]`, item.tanggal_tiket || '');
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][terminal_berangkat]`, item.terminal_berangkat || '');
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][terminal_tiba]`, item.terminal_tiba || '');
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][nominal]`, parseCurrency(item.nominal));
          formData.append(`pegawai[${pegawaiId}][transportasi][${index}][arah_perjalanan]`, item.arah_perjalanan || 'berangkat');
        });

        // Append Akomodasi
        p.akomodasi.forEach((item, index) => {
          formData.append(`pegawai[${pegawaiId}][akomodasi][${index}][jenis]`, item.jenis || 'Hotel');
          formData.append(`pegawai[${pegawaiId}][akomodasi][${index}][nama]`, item.nama || '');
          formData.append(`pegawai[${pegawaiId}][akomodasi][${index}][lokasi_hotel]`, item.lokasi_hotel || '');
          formData.append(`pegawai[${pegawaiId}][akomodasi][${index}][tanggal_checkIn]`, item.tanggal_checkIn || '');
          formData.append(`pegawai[${pegawaiId}][akomodasi][${index}][tanggal_checkOut]`, item.tanggal_checkOut || '');
          formData.append(`pegawai[${pegawaiId}][akomodasi][${index}][harga_satuan]`, parseCurrency(item.harga_satuan));
          formData.append(`pegawai[${pegawaiId}][akomodasi][${index}][malam]`, parseInt(item.malam) || 0);
          formData.append(`pegawai[${pegawaiId}][akomodasi][${index}][nominal]`, parseCurrency(item.nominal));
        });

        // Append Kontribusi
        p.kontribusi.forEach((item, index) => {
          formData.append(`pegawai[${pegawaiId}][kontribusi][${index}][jenis]`, item.jenis || 'Bimbingan Teknis');
          formData.append(`pegawai[${pegawaiId}][kontribusi][${index}][nominal]`, parseCurrency(item.nominal));
        });

        // Append Lain Lain
        p.lain_lain.forEach((item, index) => {
          formData.append(`pegawai[${pegawaiId}][lain_lain][${index}][uraian]`, item.uraian || '');
          formData.append(`pegawai[${pegawaiId}][lain_lain][${index}][tarif_satuan]`, parseCurrency(item.tarif_satuan));
          formData.append(`pegawai[${pegawaiId}][lain_lain][${index}][jumlah_hari]`, parseInt(item.jumlah_hari) || 0);
          formData.append(`pegawai[${pegawaiId}][lain_lain][${index}][nominal]`, parseCurrency(item.nominal));
          formData.append(`pegawai[${pegawaiId}][lain_lain][${index}][keterangan]`, item.keterangan || '');
        });
      });
      ...
      {view === 'list' && (
        <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-slate-800 tracking-tight">HASIL PERJALANAN DINAS</h1>
              <p className="text-sm text-slate-500">Kelola Laporan Perjalanan Dinas Jabatan dan Pembatalan Tugas.</p>
            </div>
            <div className="flex items-center gap-3 self-end sm:self-auto">
              <button 
                onClick={() => openCancelModal()} 
                className="flex items-center gap-2 px-4 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-2xl text-sm font-semibold border border-rose-100 shadow-sm transition-all duration-200"
              >
                <Ban size={16} />
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-2 gap-4">
            <div className="flex gap-2">
              <button
                onClick={() => { setActiveTab('laporan'); setPage(1); }}
              ...
              <button
                onClick={() => { setActiveTab('pembatalan'); setPage(1); }}
                className={`px-4 py-2 text-sm font-semibold rounded-xl transition-all duration-200 ${
              ...
              {activeTab === 'laporan' && (
                <div className="relative w-64">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 text-xs border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                  />
                </div>
              )}
          ...
          {loading ? (
                ...
                <table className="min-w-full divide-y divide-slate-100 text-left">
                  <thead className="bg-slate-50 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                    <tr>
                      ...
                      <th className="py-3.5 px-4">Nomor SPT Terkait</th>
                      <th className="py-3.5 px-4">Tanggal Laporan</th>
                  ...
                  <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
                    {laporanList.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="py-10 text-center text-slate-400">Belum ada laporan dibuat.</td>
                      </tr>
                    ) : (
                      laporanList.map((item, idx) => (
                        <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="py-4 px-4 text-center font-medium text-slate-400">{(page - 1) * limit + idx + 1}.</td>
                          <td className="py-4 px-4 font-semibold text-slate-800">{item.judul}</td>
                          <td className="py-4 px-4 text-slate-500 font-mono text-xs">{item.nomor_surat}</td>
                          <td className="py-4 px-4 text-slate-500">{formatDate(item.tanggal_laporan)}</td>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between border-t border-slate-100 pt-4 px-1">
                  <span className="text-xs text-slate-400">
                    Menampilkan <span className="font-semibold">{laporanList.length}</span> dari <span className="font-semibold">{totalItems}</span> laporan
                  </span>
            ...
            <div className="space-y-4">
              <div className="overflow-x-auto rounded-2xl border border-slate-100">
                <table className="min-w-full divide-y divide-slate-100 text-left">
                  <thead className="bg-slate-50 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                    <tr>
                      <th className="py-3.5 px-4 text-center w-12">No</th>
                      <th className="py-3.5 px-4">Nomor SPT</th>
                      <th className="py-3.5 px-4">Tanggal Batal</th>
                  ...
                  <tbody className="divide-y divide-slate-100 text-sm text-slate-600">
                    {canceledList.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="py-10 text-center text-slate-400">Tidak ada data pembatalan.</td>
                      </tr>
                    ) : (
                      canceledList.map((item, idx) => (
                        <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="py-4 px-4 text-center font-medium text-slate-400">{(page - 1) * limit + idx + 1}.</td>
                          <td className="py-4 px-4 text-slate-800 font-mono text-xs">{item.nomor_surat}</td>
                          <td className="py-4 px-4 text-slate-500">{formatDate(item.tanggal_pembatalan)}</td>
                          <td className="py-4 px-4 font-semibold text-slate-700">{item.pegawai_nama}</td>
                          <td className="py-4 px-4 text-slate-500 truncate max-w-[200px]" title={item.alasan}>{item.alasan || '-'}</td>
                          <td className="py-4 px-4 text-right">
                            <div className="flex items-center justify-end gap-3.5">
                              <a
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between border-t border-slate-100 pt-4 px-1">
                  <span className="text-xs text-slate-400">
                    Menampilkan <span className="font-semibold">{canceledList.length}</span> dari <span className="font-semibold">{totalItems}</span> data pembatalan
                  </span>
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => setPage(p => Math.max(1, p - 1))}
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Isi data laporan perjalanan dinas dan sesuaikan biaya realisasinya.</p>
              </div>
            </div>
            <button
          ...
          <form onSubmit={handleSubmitReport} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50/50 p-6 rounded-2xl border border-slate-100">
              <div>
                <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-2">Nomor SPT</label>
                <select
                  value={form.spt_id}
                ...
                >
                  ...
                  {sptOptions.map(opt => (
                    <option key={opt.id} value={opt.id}>{opt.nomor_surat}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-2">Tanggal Laporan</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-2">Tempat Laporan Dibuat</label>
                <input
                  type="text"
            ...
            {loadingSptDetails ? (
              <div className="py-20 flex flex-col items-center justify-center gap-3">
                <Loader2 className="w-8 h-8 text-indigo-600 animate-spin" />
                <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Mengambil Detail SPT...</p>
              </div>
            ) : selectedSpt ? (
              <>
                {/* 2. Informasi Dasar */}
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-500 mb-1.5">Judul Laporan</label>
                      <input
                        type="text"
                    ...
                    <div className="p-4 border border-slate-100 rounded-2xl bg-slate-50/30 space-y-3">
                      <p className="text-xs font-semibold text-slate-500">Pilih penandatangan laporan (otomatis dicentang dari pelaksana SPT):</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {pegawaiList.map(p => {
                          const isCanceled = (selectedSpt.pegawai_dibatalkan || []).some(cp => cp.pegawai_id === p.pegawai_id);
                          return (
                            <label 
                              key={p.pegawai_id}
                            ...
                            >
                              <input
                                ...
                                onChange={(e) => handleSignerCheckboxChange(p.pegawai_id.toString(), e.target.checked)}
                              ...
                              <div className="text-xs">
                                ...
                                <span className="text-slate-500 block">NIP. {p.nip || '-'} | {p.jabatan}</span>
                                {isCanceled && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1.5">Dasar Perjalanan Dinas (Otomatis)</label>
                        <textarea
                          readOnly
                          rows={2}
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1.5">Tujuan Perjalanan Dinas (Otomatis)</label>
                        <textarea
                          readOnly
                          rows={2}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1.5">Lama & Tanggal Perjalanan (Otomatis)</label>
                        <input
                          type="text"
                          readOnly
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1.5">Mata Anggaran yang Dipakai (Otomatis)</label>
                        <input
                          type="text"
                          readOnly
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-500 mb-1.5">Deskripsi Perjalanan (Secara Kronologis)</label>
                      <textarea
                        rows={4}
                        placeholder="Uraikan kronologis perjalanan dinas..."
                        value={form.deskripsi_kronologis}
                        onChange={(e) => setForm(prev => ({ ...prev, deskripsi_kronologis: e.target.value }))}
                        className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-white text-slate-800 shadow-sm"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-500 mb-1.5">Tempat yang Dikunjungi (Otomatis)</label>
                      <input
                        type="text"
                        readOnly
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-500 mb-1.5">Hasil yang Dicapai</label>
                      <textarea
                        rows={4}
                        placeholder="Hasil yang dicapai selama melakukan perjalanan dinas..."
                        value={form.hasil_dicapai}
                        onChange={(e) => setForm(prev => ({ ...prev, hasil_dicapai: e.target.value }))}
                        className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-white text-slate-800 shadow-sm"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-6 pt-4 border-t border-slate-100">
                    ...
                    <div className="space-y-8">
                      {selectedSigners.map(signerId => {
                        ...
                        return (
                          <div key={signerId} className="border border-slate-100 rounded-3xl p-5 md:p-6 bg-slate-50/20 space-y-6">
                            <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
                              <h4 className="text-md font-bold text-indigo-700">Rincian Pengeluaran Untuk: {peg.nama_lengkap}</h4>
                            ...
                            <div className="space-y-4">
                              <div className="flex items-center justify-between">
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Transportasi (Pergi & Pulang)</label>
                                <button
                                  type="button"
                              ...
                              <div className="space-y-3">
                                {expData.transportasi.map((row, idx) => (
                                  <div key={idx} className="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 bg-white border border-slate-100 rounded-2xl relative">
                                    <div className="md:col-span-2">
                                      <select
                                        value={row.jenis}
                                        onChange={(e) => handleExpenseInputChange(signerId, 'transportasi', idx, 'jenis', e.target.value)}
                                    ...
                                    <div className="md:col-span-2">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'transportasi', idx, 'perusahaan', e.target.value)}
                                    ...
                                    <div className="md:col-span-2">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'transportasi', idx, 'kode_boking', e.target.value)}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl"
                                      />
                                    </div>
                                    <div className="md:col-span-2">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'transportasi', idx, 'nomor_tiket', e.target.value)}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl"
                                      />
                                    </div>
                                    <div className="md:col-span-2">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'transportasi', idx, 'tanggal_tiket', e.target.value)}
                                    ...
                                    <div className="md:col-span-2">
                                      <input
                                        ...
                                        value={row.nominal}
                                        onChange={(e) => handleExpenseInputChange(signerId, 'transportasi', idx, 'nominal', formatCurrency(e.target.value))}
                                    ...
                                    <div className="md:col-span-3">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'transportasi', idx, 'terminal_berangkat', e.target.value)}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl"
                                      />
                                    </div>
                                    <div className="md:col-span-3">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'transportasi', idx, 'terminal_tiba', e.target.value)}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl"
                                      />
                                    </div>
                                      ...
                                      <div className="flex gap-3">
                                        <label className="flex items-center gap-1 text-[11px] text-slate-600">
                                          <input
                                            ...
                                            onChange={() => handleExpenseInputChange(signerId, 'transportasi', idx, 'arah_perjalanan', 'berangkat')}
                                        ...
                                        <label className="flex items-center gap-1 text-[11px] text-slate-600">
                                          <input
                                            ...
                                            onChange={() => handleExpenseInputChange(signerId, 'transportasi', idx, 'arah_perjalanan', 'kembali')}
                                            className="text-indigo-600"
                                          />
                                          <span>Kembali</span>
                                        </label>
                                    ...
                                    {expData.transportasi.length > 1 && (
                                      <button
                                        type="button"
                                        onClick={() => removeExpenseRow(signerId, 'transportasi', idx)}
                            ...
                            <div className="space-y-4 pt-4 border-t border-slate-100">
                              <div className="flex items-center justify-between">
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Akomodasi / Penginapan</label>
                                <button
                                  type="button"
                                ...
                                {expData.akomodasi.map((row, idx) => (
                                  <div key={idx} className="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 bg-white border border-slate-100 rounded-2xl relative">
                                    <div className="md:col-span-3">
                                      <select
                                        value={row.jenis}
                                        onChange={(e) => handleExpenseInputChange(signerId, 'akomodasi', idx, 'jenis', e.target.value)}
                                        className="w-full text-xs px-2 py-2 border border-slate-200 rounded-xl"
                                      >
                                        <option>Hotel</option>
                                    ...
                                    <div className="md:col-span-3">
                                      <input
                                        ...
                                        value={row.nama}
                                        onChange={(e) => handleExpenseInputChange(signerId, 'akomodasi', idx, 'nama', e.target.value)}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl"
                                      />
                                    </div>
                                    <div className="md:col-span-3">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'akomodasi', idx, 'lokasi_hotel', e.target.value)}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl"
                                      />
                                    </div>
                                    <div className="md:col-span-3">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'akomodasi', idx, 'harga_satuan', formatCurrency(e.target.value))}
                                        className={`w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl text-right font-medium ${
                                          row.readOnlyHarga ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : 'text-slate-800'
                                        }`}
                                      />
                                    </div>
                                    <div className="md:col-span-3">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'akomodasi', idx, 'tanggal_checkIn', e.target.value)}
                                        className="w-full text-xs px-2 py-2 border border-slate-200 rounded-xl text-slate-600"
                                      />
                                    </div>
                                    <div className="md:col-span-3">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'akomodasi', idx, 'tanggal_checkOut', e.target.value)}
                                        className="w-full text-xs px-2 py-2 border border-slate-200 rounded-xl text-slate-600"
                                      />
                                    </div>
                                    <div className="md:col-span-2 flex items-center gap-1.5">
                                      <input
                                        ...
                                        placeholder="Qty"
                                        value={row.malam}
                                        onChange={(e) => handleExpenseInputChange(signerId, 'akomodasi', idx, 'malam', e.target.value)}
                            ...
                            <div className="space-y-4 pt-4 border-t border-slate-100">
                              <div className="flex items-center justify-between">
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Kontribusi (BIMTEK/Diklat/Kursus)</label>
                                <button
                                  type="button"
                                ...
                                {expData.kontribusi.map((row, idx) => (
                                  <div key={idx} className="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 bg-white border border-slate-100 rounded-2xl relative">
                                    <div className="md:col-span-6">
                                      <select
                                        value={row.jenis}
                                        onChange={(e) => handleExpenseInputChange(signerId, 'kontribusi', idx, 'jenis', e.target.value)}
                                        className="w-full text-xs px-2 py-2 border border-slate-200 rounded-xl"
                                      >
                                    ...
                                    <div className="md:col-span-6 flex items-center gap-2">
                                      <input
                                        ...
                                        placeholder="Nominal (Rp)"
                                        value={row.nominal}
                                        onChange={(e) => handleExpenseInputChange(signerId, 'kontribusi', idx, 'nominal', formatCurrency(e.target.value))}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl text-right font-medium text-slate-800"
                                      />
                                    ...
                                    {expData.kontribusi.length > 1 && (
                                      <button
                                        type="button"
                                        onClick={() => removeExpenseRow(signerId, 'kontribusi', idx)}
                                        className="absolute -top-2.5 -right-2.5 p-1 bg-white border border-slate-100 hover:border-rose-200 text-rose-500 hover:text-rose-700 rounded-full shadow-sm transition-all"
                                      >
                                        <X size={12} />
                                      </button>
                            ...
                            <div className="space-y-4 pt-4 border-t border-slate-100">
                              <div className="flex items-center justify-between">
                                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Pengeluaran Lain-lain (Sewa Mobil, dll)</label>
                                <button
                                  type="button"
                                  ...
                                  <div key={idx} className="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 bg-white border border-slate-100 rounded-2xl relative">
                                    <div className="md:col-span-3">
                                      <input
                                        ...
                                        value={row.uraian}
                                        onChange={(e) => handleExpenseInputChange(signerId, 'lain_lain', idx, 'uraian', e.target.value)}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl"
                                      />
                                    </div>
                                    <div className="md:col-span-2">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'lain_lain', idx, 'tarif_satuan', formatCurrency(e.target.value))}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl text-right font-medium text-slate-800"
                                      />
                                    </div>
                                    <div className="md:col-span-2 flex items-center gap-1.5">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'lain_lain', idx, 'jumlah_hari', e.target.value)}
                                        className="w-full text-xs px-2 py-2 border border-slate-200 rounded-xl text-center font-bold"
                                      />
                                    ...
                                    <div className="md:col-span-3">
                                      <input
                                        ...
                                        onChange={(e) => handleExpenseInputChange(signerId, 'lain_lain', idx, 'keterangan', e.target.value)}
                                        className="w-full text-xs px-2.5 py-2 border border-slate-200 rounded-xl"
                                      />
                                    </div>
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <div className="flex items-center gap-2 pb-1 border-b border-slate-100">
                    ...
                    <h3 className="text-md font-bold text-slate-800">5. Lampiran (Bukti Perjalanan)</h3>
                  </div>
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-400 mt-1">Format PNG, JPG, PDF (Maksimal 10MB per file)</p>
                  </div>

                  {/* List of files */}
                  {(existingFiles.length > 0 || newFiles.length > 0) && (
                    <div className="mt-4 space-y-3">
                      <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Daftar Lampiran:</p>
                      
                      {/* Existing files */}
                      {existingFiles.map(file => {
                        const isImage = file.file_name?.match(/\.(jpeg|jpg|png|gif|webp)$/i);
                        return (
                          <div key={file.id} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-150 rounded-xl">
                            <div className="flex items-center gap-3">
                              {isImage ? (
                                <img
                                  src={`/${file.file_path}`}
                              ...
                              <div>
                                ...
                                <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Tersimpan di server</span>
                              </div>
                            </div>
                            <button
                              type="button"
                        ...
                        return (
                          <div key={idx} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-xl">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <span className="text-sm font-semibold text-slate-700 truncate block max-w-xs">{file.name}</span>
                                <span className="text-[10px] text-emerald-600 font-semibold uppercase tracking-wider block">Siap diupload</span>
                              </div>
                            </div>
                            <button
                              type="button"
          ...
          <div className="w-full max-w-lg bg-white rounded-3xl border border-slate-100 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="px-6 py-5 bg-gradient-to-r from-red-600 to-rose-500 text-white flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-red-100 mt-0.5">Batalkan tugas pegawai pada SPT dan kelola biaya hangus.</p>
              </div>
              <button 
            ...
            {/* Modal Body */}
            <form onSubmit={handleCancelSubmit} className="flex-1 overflow-y-auto p-6 space-y-4 text-left">
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">Nomor SPT yang Dibatalkan</label>
                <select
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">Pegawai yang Dibatalkan</label>
                <select
                ...
                >
                  {loadingCancelPegawai ? (
                    ...
                    <option value="">{cancelForm.spt_id ? '-- Semua pegawai sudah melapor --' : '-- Pilih SPT terlebih dahulu --'}</option>
                  ) : (
                    <>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Tempat Pembatalan</label>
                  <input
                    type="text"
                    value={cancelForm.tempat_pembatalan}
                    onChange={(e) => setCancelForm(prev => ({ ...prev, tempat_pembatalan: e.target.value }))}
                    required
                    className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Tanggal Pembatalan</label>
                  <input
                    type="date"
                    value={cancelForm.tanggal_pembatalan}
                    onChange={(e) => setCancelForm(prev => ({ ...prev, tanggal_pembatalan: e.target.value }))}
                    required
                    className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-500"
                  />
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">Alasan Pembatalan</label>
                <textarea
                  rows={3}
              ...
              <div className="border-t border-slate-100 pt-4 space-y-3">
                <div className="flex items-center justify-between">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Perincian Biaya Hangus</label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-xs text-slate-500 font-semibold">Ambil dari Uang Muka</span>
                  </label>
                </div>
                <div className="space-y-3">
                  <input
                    ...
                    onChange={(e) => setCancelForm(prev => ({ ...prev, rincian_biaya: e.target.value }))}
                    className={`w-full px-3 py-2 border border-slate-200 rounded-xl text-sm ${
                      cancelForm.ambilPanjar ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : 'bg-white'
                    }`}
                  />
                  <input
                    type="text"
                    ...
                    readOnly={cancelForm.ambilPanjar}
                    onChange={(e) => setCancelForm(prev => ({ ...prev, nominal_biaya: formatCurrency(e.target.value) }))}
                    className={`w-full px-3 py-2 border border-slate-200 rounded-xl text-sm text-right font-medium ${
                      cancelForm.ambilPanjar ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : 'bg-white text-slate-800'
                    }`}
                  />
                </div>
                <p className="text-[10px] text-slate-400 font-medium">
                  Isi manual jika tidak ada uang muka, atau aktifkan checkbox untuk mengambil data otomatis dari data Panjar yang ada.
                </p>
              </div>
