...
const Profil = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      setPreviewUrl(user.foto_profil ? `/${user.foto_profil}` : '');
    }
  }, [user]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500">Lihat dan perbarui informasi pribadi serta pengaturan kepegawaian Anda.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center text-center space-y-4">
          ...
          <div className="w-full pt-4 border-t border-slate-100 dark:border-slate-700 flex flex-col gap-2.5">
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 bg-slate-50 dark:bg-slate-900/50 dark:text-slate-400 px-3.5 py-2 rounded-xl">
              ...
              <span>Hak Akses: <strong className="capitalize text-slate-700 dark:text-slate-300">{user?.role}</strong></span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-6">
          <div className="border-b border-slate-100 dark:border-slate-700 pb-3">
            <h3 className="text-base font-bold text-slate-850 dark:text-slate-50">Detail Informasi Akun</h3>
            <p className="text-xs text-slate-400">Silakan ubah field di bawah ini kemudian klik Simpan Perubahan.</p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {user?.role === 'superadmin' && (
                  <p className="text-[10px] text-amber-600 dark:text-amber-400 mt-1 font-medium">Username Super Admin tidak dapat diubah demi alasan keamanan.</p>
                )}
              </div>
