...
const Dashboard = () => {
    ...
    const realisasiPersen = totalNilaiAnggaran > 0 ? ((totalRealisasi / totalNilaiAnggaran) * 100).toFixed(1) : 0;

    const donutData = {
        labels: [
            ...anggaranList.filter(a => a.realisasi > 0).map(a => `Realisasi ${a.mata_anggaran_nama}`),
            'Sisa Anggaran'
        ],
        datasets: [{
            data: [
            ...
            backgroundColor: [
                ...
                '#858796', '#6f42c1', '#fd7e14', '#0284c7', '#4b5563',
                '#e2e8f0' // Warna abu untuk sisa anggaran (item terakhir)
            ].slice(0, anggaranList.filter(a => a.realisasi > 0).length).concat('#e2e8f0'),
            borderWidth: 2,
            borderColor: '#ffffff',
            hoverOffset: 4
        }]
    ...
    const lineData = {
        labels: stats?.grafikPerjalanan?.labels?.map(l => {
            const parts = l.split('-');
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between group hover:shadow-md transition-all">
                    <div className="space-y-1">
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Perjalanan</p>
                        <h3 className="text-3xl font-extrabold text-slate-800">{stats?.totalPerjalanan || 0}</h3>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between group hover:shadow-md transition-all">
                    <div className="space-y-1">
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Perjalanan Bulan Ini</p>
                        <h3 className="text-3xl font-extrabold text-amber-600">{stats?.perjalananBulanIni || 0}</h3>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between group hover:shadow-md transition-all">
                    <div className="space-y-1">
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Realisasi Keuangan</p>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-between min-h-[400px]">
                    ...
                    <div className="flex-1 relative flex items-center justify-center min-h-[220px]">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
                            ...
                            <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Telah Terealisasi</span>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-4 mt-4 text-center">
                        <div>
                            ...
                            <p className="text-sm font-bold text-slate-800">Rp {new Intl.NumberFormat('id-ID').format(totalRealisasi)}</p>
                        </div>
                        <div>
                            <p className="text-[10px] font-semibold text-slate-400 uppercase">Sisa Anggaran</p>
                            <p className="text-sm font-bold text-slate-500">Rp {new Intl.NumberFormat('id-ID').format(totalSisaAnggaran)}</p>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                <h4 className="text-base font-bold text-slate-800 mb-4">Pegawai yang Sedang Berdinas Bulan Ini</h4>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-slate-100">
                            <thead className="bg-slate-50/50">
                                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider w-16">No</th>
                                    <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Pegawai</th>
                                    <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">NIP</th>
                                    <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nomor SPT</th>
                                    <th className="px-4 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Periode Dinas</th>
                                </tr>
                            </thead>
                        ...
                        {totalPages > 1 && (
                            <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-4">
                                <span className="text-xs text-slate-400">Menampilkan {paginatedPegawai.length} dari {onDutyPegawai.length} data</span>
