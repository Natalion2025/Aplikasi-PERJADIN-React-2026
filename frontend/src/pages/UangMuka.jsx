...
const UangMuka = () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        setForm({
          ...
          rincian: panjar.rincian && panjar.rincian.length > 0
  ...
  const handleSubmit = async (e) => {
    ...
    const isRincianValid = form.rincian.every(item => item.uraian.trim() !== '' && item.jumlah > 0);
    if (!isRincianValid) {
  ...
  const handleDelete = async (id, pelaksana) => {
    if (!window.confirm(`Apakah Anda yakin ingin menghapus data uang muka pelaksana "${pelaksana}"?`)) {
      return;
    }
    try {
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white rounded-xl font-semibold shadow-md shadow-indigo-600/15 transition-all text-sm transform hover:-translate-y-0.5 self-start md:self-auto"
        >
        ...
        <div className="p-0">
              ...
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50 text-slate-500 font-semibold dark:border-slate-700/50 dark:bg-slate-800/50 dark:text-slate-400">
                    ...
                    <th className="py-4 px-4 w-44">Tgl Panjar</th>
                    <th className="py-4 px-4">Surat Tugas Terkait</th>
                    <th className="py-4 px-4">Pegawai Pelaksana</th>
                    <th className="py-4 px-4 text-right w-44">Total Biaya</th>
                    <th className="py-4 px-6 text-right w-36">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                  {panjarList.map((panjar, idx) => (
                    <tr key={panjar.id} className="hover:bg-slate-50/40 dark:hover:bg-slate-700/20 transition-all">
                      <td className="py-4 px-6 text-center text-slate-500 dark:text-slate-400 font-medium">
                        {(page - 1) * limit + idx + 1}.
                      </td>
                      <td className="py-4 px-4 text-slate-650 dark:text-slate-355 font-medium">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between p-5 border-t border-slate-100 dark:border-slate-700/50 bg-slate-50/30 dark:bg-slate-800/10 gap-4">
            <span className="text-sm text-slate-500 dark:text-slate-400">
              Menampilkan Halaman <span className="font-semibold text-slate-800 dark:text-slate-200">{page}</span> dari <span className="font-semibold text-slate-800 dark:text-slate-200">{totalPages}</span> ({totalItems} total data)
            </span>
            ...
            <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-850">
              <div>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white">
                  {editId ? 'Edit Uang Muka (Panjar) Perjalanan' : 'Tambah Uang Muka (Panjar) Perjalanan'}
                </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setForm(prev => ({ ...prev, tanggal_panjar: e.target.value }))}
                        className="w-full px-4 py-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setForm(prev => ({ ...prev, pelaksana_id: e.target.value }))}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setForm(prev => ({ ...prev, bendahara_id: e.target.value }))}
                        className="w-full px-4 py-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600"
                      >
