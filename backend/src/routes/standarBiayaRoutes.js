...
router.get("/standar-biaya", isApiAuthenticated, async (req, res) => {
  try {
    const rows = await dbAll(
      "SELECT * FROM standar_biaya ORDER BY tipe_biaya, id",
      [],
    );
    res.json(rows);
  } catch (err) {
...
router.get("/standar-biaya/:tipe", isApiAuthenticated, async (req, res) => {
  try {
    const rows = await dbAll(
      "SELECT * FROM standar_biaya WHERE tipe_biaya = ? ORDER BY id",
      [req.params.tipe],
    );
    res.json(rows);
  } catch (err) {
 ...
 */
const runQueryWithRetry = (sql, params = []) =>
  new Promise((resolve, reject) => {
    const tryQuery = (retries = 5) => {
      db.run(sql, params, function (err) {
        if (err && err.code === "SQLITE_BUSY" && retries > 0) {
          console.warn(
            `[DB WARN] Database sibuk, mencoba lagi... Sisa percobaan: ${retries - 1} `,
          );
  ...
  async (req, res) => {
    ...
    try {
      ...
      worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
        ...
        if (rowNumber > headerRowsToSkip) {
          ...
          row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
            rowData[colNumber] =
              cell.value !== null && cell.value !== undefined ? cell.text : "";
          });
      ...
      await runQueryWithRetry(
        "DELETE FROM standar_biaya WHERE tipe_biaya = ?",
        [tipeBiaya],
      );
...
VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      ...
      res.json({
        message: "Data standar biaya berhasil diupload dan disimpan.",
      });
    } catch (error) {
      ...
      res
        .status(500)
        .json({ message: "Terjadi kesalahan saat memproses file Excel." });
    }
  },
);
...
router.get("/standar-biaya/template/:tipe", (req, res) => {
  ...
  if (!fileName) {
    return res
      .status(404)
      .send(`Template untuk tipe ${tipeBiaya} tidak didefinisikan.`);
  }
  ...
  if (fs.existsSync(filePath)) {
    ...
    console.error(
      `[DOWNLOAD ERROR] Template tidak ditemukan di path: ${filePath} `,
    );
...
router.get(
  "/spt/:spt_id/accommodation-standards",
  isApiAuthenticated,
  async (req, res) => {
    ...
    `;
      ...
      const cariStandarAkomodasi = async (tipeBiaya, lokasiQuery) => {
        ...
        const cleanLokasiQuery = lokasiQuery
          .replace(/kabupaten|kota|kab\.?/gi, "")
          .trim();
        ...
        if (tipeBiaya === "B") {
          ...
          if (lokasiLower.includes("dki") || lokasiLower.includes("jakarta")) {
            console.log(
              `[DEBUG] Kueri Jakarta terdeteksi.Mengubah pencarian menjadi '%JAKARTA%'`,
            );
            params = ["%JAKARTA%"];
          }
        ...
        if (!result) {
          ...
          const fallbackQuery =
            tipeBiaya === "B"
              ? `SELECT * FROM standar_biaya WHERE tipe_biaya = 'B' AND TRIM(UPPER(uraian)) LIKE TRIM(UPPER(?))`
              : `SELECT * FROM standar_biaya WHERE tipe_biaya = 'E' AND TRIM(UPPER(provinsi)) LIKE TRIM(UPPER(?))`;
      ...
      if (infoLokasi.jenis === "desa" || infoLokasi.jenis === "kecamatan") {
        ...
        if (isSameRegion) {
          ...
          standarAkomodasi = await cariStandarAkomodasi(
            "E",
            infoLokasi.provinsi,
          );
        }
        ...
        standarAkomodasi = await cariStandarAkomodasi(
          "E",
          spt.lokasi_tujuan.split(",").pop().trim(),
        );
      }
      ...
      if (!standarAkomodasi) {
        ...
        console.warn(
          `[WARN] Standar biaya akomodasi untuk "${spt.lokasi_tujuan}" tidak ditemukan.Menggunakan fallback...`,
        );
        standarAkomodasi = await dbGet(
          `SELECT * FROM standar_biaya WHERE tipe_biaya = 'E' OR tipe_biaya = 'B' ORDER BY id LIMIT 1`,
        );
        if (!standarAkomodasi) {
          return res.status(404).json({
            message: `Standar biaya akomodasi untuk lokasi "${spt.lokasi_tujuan}" tidak ditemukan, dan tidak ada data standar biaya fallback yang tersedia.`,
          });
        }
      }
      ...
      console.error(
        `[API ERROR] Gagal mengambil standar akomodasi untuk SPT ID ${spt_id}: `,
        error,
      );
      res.status(500).json({
