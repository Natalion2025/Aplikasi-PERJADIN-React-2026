...
router.get("/pengeluaran-riil", isApiAuthenticated, async (req, res) => {
...
router.post("/pengeluaran-riil", isApiAuthenticated, async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      message: "Data pengeluaran riil berhasil disimpan!",
      id: result.lastID,
    });
  } catch (error) {
...
router.get("/pengeluaran-riil/:id", isApiAuthenticated, async (req, res) => {
  try {
    ...
    if (!item) {
      return res
        .status(404)
        .json({ message: "Data pengeluaran riil tidak ditemukan." });
    }
    res.json(item);
  } catch (error) {
    console.error(
      `[API ERROR] Gagal mengambil pengeluaran riil id ${req.params.id}: `,
      error,
    );
    res.status(500).json({ message: "Terjadi kesalahan pada server." });
  }
...
router.put("/pengeluaran-riil/:id", isApiAuthenticated, async (req, res) => {
  const { id } = req.params;
  const { spt_id, pegawai_id, uraian, jumlah } = req.body;
  ...
  try {
    ...
    if (result.changes === 0) {
      return res
        .status(404)
        .json({ message: "Data tidak ditemukan untuk diperbarui." });
    }
    res.json({ message: "Data pengeluaran riil berhasil diperbarui." });
  } catch (error) {
    console.error(
      `[API ERROR] Gagal memperbarui pengeluaran riil id ${id}: `,
      error,
    );
    res
      .status(500)
...
router.delete("/pengeluaran-riil/:id", isApiAuthenticated, async (req, res) => {
  try {
    const result = await runQuery("DELETE FROM pengeluaran_riil WHERE id = ?", [
      req.params.id,
    ]);
    if (result.changes === 0) {
    ...
    console.error(
      `[API ERROR] Gagal menghapus pengeluaran riil id ${req.params.id}: `,
      err,
    );
    res
      .status(500)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const allRiilItems = await dbAll(allRiilSql, [
        mainRiil.spt_id,
        mainRiil.pegawai_id,
      ]);
      ...
      console.error(
        `[API ERROR] Gagal mengambil data cetak pengeluaran riil id ${req.params.id}: `,
        error,
      );
      res.status(500).json({
...
router.get("/cetak/pembayaran/:id", isApiAuthenticated, async (req, res) => {
  const { id } = req.params;
  try {
    // 1. Ambil data utama pembayaran
    const pembayaran = await dbGet(`SELECT * FROM pembayaran WHERE id = ? `, [
      id,
    ]);
    if (!pembayaran) {
      return res
    ...
    const anggaran = await dbGet(`SELECT * FROM anggaran WHERE id = ? `, [
      pembayaran.anggaran_id,
    ]);

    // 3. Ambil data SPT terkait
    const spt = await dbGet(
      `SELECT nomor_surat, tanggal_surat FROM spt WHERE id = ? `,
      [pembayaran.spt_id],
    );

    // 4. Ambil data rincian dari laporan (sama seperti di modal)
    const laporanResponse = await fetch(
      `http://localhost:${PORT}/api/laporan/by-spt/${pembayaran.spt_id}`,
      {
    ...
    if (!laporanResponse.ok) {
      throw new Error(
        "Gagal mengambil rincian pengeluaran dari laporan terkait.",
      );
    }
    ...
    const getPegawaiDefaultCosts = (pegId) => {
      ...
      const exp =
        laporanData.pengeluaran.find(
          (p) => p.pegawai_id.toString() === pegId,
        ) || {};
      if (!pInfo) return [];
      ...
      costs.push({
        type: "uang_harian",
        total: (pInfo.uang_harian?.harga_satuan || 0) * jumlahHariHarian,
      });

      if (pInfo.biaya_representasi && pInfo.biaya_representasi.harga > 0) {
        costs.push({
          type: "representasi",
          total: pInfo.biaya_representasi.harga * jumlahHariHarian,
        });
      }

      if (exp.transportasi_nominal > 0)
        costs.push({ type: "transportasi", total: exp.transportasi_nominal });
      if (exp.akomodasi_nominal > 0)
        costs.push({ type: "akomodasi", total: exp.akomodasi_nominal });
      if (exp.kontribusi_nominal > 0)
        costs.push({ type: "kontribusi", total: exp.kontribusi_nominal });
      if (exp.lain_lain_nominal > 0)
        costs.push({ type: "lain_lain", total: exp.lain_lain_nominal });
    ...
    laporanData.pengeluaran.forEach((pengeluaran) => {
      ...
      const penerimaInfo = laporanData.penerima.find(
        (p) => p.id.toString() === pegawaiId,
      );
      if (!penerimaInfo) return;

      const jumlahHariHarian = (pengeluaran.akomodasi_malam || 0) + 1;
      const uangHarianTotal =
        (penerimaInfo.uang_harian.harga_satuan || 0) * jumlahHariHarian;
      addBiaya(
        pegawaiId,
        `Uang Harian (${penerimaInfo.uang_harian.golongan})`,
        penerimaInfo.uang_harian.harga_satuan,
        penerimaInfo.uang_harian.satuan,
        jumlahHariHarian,
        uangHarianTotal,
        "uang_harian",
      );
      ...
      if (
        penerimaInfo.biaya_representasi &&
        penerimaInfo.biaya_representasi.harga > 0
      ) {
        const jumlahHariRepresentasi = (pengeluaran.akomodasi_malam || 0) + 1;
        const totalBiayaRepresentasi =
          penerimaInfo.biaya_representasi.harga * jumlahHariRepresentasi;
        const uraianRepresentasi = `${penerimaInfo.biaya_representasi.uraian} (Biaya Representasi)`;
        addBiaya(
          pegawaiId,
          uraianRepresentasi,
          penerimaInfo.biaya_representasi.harga,
          penerimaInfo.biaya_representasi.satuan,
          jumlahHariRepresentasi,
          totalBiayaRepresentasi,
          "representasi",
        );
      }

      if (pengeluaran.transportasi_nominal > 0)
        addBiaya(
          pegawaiId,
          `Biaya Transportasi (${pengeluaran.transportasi_jenis || "N/A"})`,
          pengeluaran.transportasi_nominal,
          "PP",
          "-",
          pengeluaran.transportasi_nominal,
          "transportasi",
        );
      if (pengeluaran.akomodasi_nominal > 0)
        addBiaya(
          pegawaiId,
          `Biaya Akomodasi (${pengeluaran.akomodasi_jenis || "N/A"})`,
          pengeluaran.akomodasi_harga_satuan,
          "Malam",
          pengeluaran.akomodasi_malam,
          pengeluaran.akomodasi_nominal,
          "akomodasi",
        );
      if (pengeluaran.kontribusi_nominal > 0)
        addBiaya(
          pegawaiId,
          `Biaya Kontribusi (${pengeluaran.kontribusi_jenis || "N/A"})`,
          pengeluaran.kontribusi_nominal,
          "OK",
          "-",
          pengeluaran.kontribusi_nominal,
          "kontribusi",
        );
      if (pengeluaran.lain_lain_nominal > 0)
        addBiaya(
          pegawaiId,
          `Biaya Lain-lain (${pengeluaran.lain_lain_uraian || "N/A"})`,
          pengeluaran.lain_lain_nominal,
          "OK",
          "-",
          pengeluaran.lain_lain_nominal,
          "lain_lain",
    ...
    const bendahara = await dbGet(
      "SELECT nama_lengkap, nip FROM pegawai WHERE jabatan LIKE '%Bendahara Pengeluaran%' LIMIT 1",
    );
    // PERBAIKAN: Ambil data PPTK berdasarkan pptk_id yang tersimpan di pembayaran
    const pptk = pembayaran.pptk_id
      ? await dbGet("SELECT nama_lengkap, nip FROM pegawai WHERE id = ?", [
          pembayaran.pptk_id,
        ])
      : await dbGet(
          "SELECT nama_lengkap, nip FROM pegawai WHERE jabatan LIKE '%PPTK%' OR jabatan LIKE '%Pejabat Pelaksana Teknis Kegiatan%' LIMIT 1",
        );
    const penggunaAnggaran = await dbGet(
    ...
    console.error(
      `[API ERROR] Gagal membuat data cetak untuk pembayaran id ${id}:`,
      error,
    );
    res.status(500).json({
      message: "Terjadi kesalahan pada server.",
