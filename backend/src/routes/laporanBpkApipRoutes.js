...
router.get("/api/laporan-bpk-apip", isApiAuthenticated, async (req, res) => {
  ...
  const offset = usePagination
    ? (page - 1) * (parseInt(req.query.limit, 10) || 5)
    : 0; // Offset default 0
  ...
  try {
    ...
    const totalResult = await dbGet(
      "SELECT COUNT(*) as total FROM spt_pegawai",
      [],
    );
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(
        "[API ERROR] Gagal mengambil data transportasi BPK & APIP:",
        err,
      );
      res.status(500).json({ message: err.message });
    }
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(
        "[API ERROR] Gagal mengambil data akomodasi BPK & APIP:",
        err,
      );
      res.status(500).json({ message: err.message });
    }
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const item of allItems) {
        ...
        const isDalamKota =
          ["desa", "kecamatan"].includes(infoLokasi.jenis) ||
          (infoLokasi.jenis === "kabupaten" &&
            infoLokasi.nama
              .toLowerCase()
              .includes((item.tempat_berangkat || "").toLowerCase()));

        const standarBiayaHarian = await cariStandarBiaya(
          isDalamKota ? "A" : "C",
          isDalamKota
            ? infoLokasi.nama
            : infoLokasi.provinsi || infoLokasi.nama,
        );

        const tingkatBiaya = getTingkatBiaya(item);
        const kolomGolongan = getKolomGolongan(tingkatBiaya);
        const tarifSatuan = standarBiayaHarian
          ? standarBiayaHarian[kolomGolongan] || standarBiayaHarian.besaran || 0
          : 0;
        ...
        if (
          item.jabatan &&
          item.jabatan.toLowerCase().includes("kepala dinas") &&
          biayaRepresentasiEselonII
        ) {
          const tarifRepresentatif = isDalamKota
      ...
      const paginatedData = usePagination
        ? processedData.slice(offset, offset + limitNum)
        : processedData;
      ...
      console.error(
        "[API ERROR] Gagal mengambil data Uang Harian BPK & APIP:",
        err,
      );
      res.status(500).json({ message: err.message });
    }
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(
        "[API ERROR] Gagal mengambil data Biaya Lain-lain BPK & APIP:",
        err,
      );
      res.status(500).json({ message: err.message });
    }
...
router.get("/api/cetak/laporan-bpk", isApiAuthenticated, async (req, res) => {
  try {
    ...
    const cariJenisLokasi = (lokasi, tempatBerangkat) => {
      ...
      for (const group of locationsData) {
        if (group.group.toLowerCase().includes("kecamatan")) {
          for (const loc of group.locations) {
            if (
              loc.toLowerCase().includes(lokasiLower) ||
              lokasiLower.includes(loc.toLowerCase())
            )
              return "Dalam Daerah";
          }
          if (
      ...
      if (lokasiLower.includes(tempatBerangkat.toLowerCase()))
        return "Dalam Daerah";
      return "Luar Daerah";
    };
    ...
    for (const item of allItems) {
      ...
      if (item.laporan_id) {
        ...
        const allTransport =
          (await dbAll(
            "SELECT * FROM laporan_transportasi WHERE laporan_id = ? AND pegawai_id = ? ORDER BY id ASC",
            [item.laporan_id, item.pegawai_id],
          )) || [];
        ...
        akomodasi = (await dbGet(
          "SELECT * FROM laporan_akomodasi WHERE laporan_id = ? AND pegawai_id = ?",
          [item.laporan_id, item.pegawai_id],
        )) || { nominal: 0 };
      }

      const { data: uangHarianData } = await fetch(
        `http://localhost:${PORT}/api/laporan-bpk-apip/uang-harian?limit=0`,
        { headers: { Cookie: req.headers.cookie } },
      ).then((res) => res.json());
      const uhDetail = uangHarianData.find(
        (uh) => uh.spt_id === item.spt_id && uh.pegawai_id === item.pegawai_id,
      );
      ...
      const semuaBiayaLain = item.laporan_id
        ? await dbAll(
            "SELECT * FROM laporan_lain_lain WHERE laporan_id = ? AND pegawai_id = ?",
            [item.laporan_id, item.pegawai_id],
          )
        : [];
      ...
      const rentalItems = semuaBiayaLain.filter(
        (biaya) =>
          biaya.uraian &&
          rentalKeywords.some((keyword) =>
            biaya.uraian.toLowerCase().includes(keyword),
          ),
      );
      const otherItems = semuaBiayaLain.filter(
        (biaya) =>
          !biaya.uraian ||
          !rentalKeywords.some((keyword) =>
            biaya.uraian.toLowerCase().includes(keyword),
      ...
      results.push({
        ...item,
        jenis_perjadin: cariJenisLokasi(
          item.lokasi_tujuan,
    ...
    res
      .status(500)
      .json({ message: "Gagal mengambil data laporan.", error: error.message });
  }
});
