...
router.get("/spt/active-count", isApiAuthenticated, async (req, res) => {
  try {
    ...
    const pegawai = await dbGet("SELECT id FROM pegawai WHERE nip = ?", [
      userNip,
    ]);
    if (!pegawai) {
      return res.json({ count: 0 });
    ...
    console.error(
      "[API NOTIF ERROR] Gagal mengambil hitungan SPT aktif:",
      error,
    );
    res
      .status(500)
      .json({ message: "Terjadi kesalahan server saat mengambil notifikasi." });
  }
});
...
router.get("/spt", isApiAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const totalSql =
      "SELECT COUNT(*) as total FROM spt WHERE status != 'dibatalkan'";
...
router.get("/spt/canceled", isApiAuthenticated, async (req, res) => {
  ...
  try {
    ...
    console.error(
      "[API ERROR] Gagal mengambil daftar SPT yang dibatalkan:",
      err,
    );
    res.status(500).json({ message: "Terjadi kesalahan pada server." });
  }
...
router.get("/spt/:id", isApiAuthenticated, async (req, res) => {
  try {
    ...
    const canceledPegawaiSql = `
            ...
            WHERE ps.spt_id = ?
            `;
    const canceledPegawaiRows = await dbAll(canceledPegawaiSql, [
      req.params.id,
    ]);
...
router.post("/spt", isApiAuthenticated, async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      message: "Data tidak lengkap. Harap isi semua kolom yang wajib diisi.",
    });
  }

  const hasNonFollower = pegawai.some(
    (p) => p.pengikut === "0" || p.pengikut === 0,
  );
  if (!hasNonFollower) {
    return res.status(400).json({
      message:
        "Harus ada minimal satu pegawai yang ditugaskan (bukan pengikut).",
    });
  }
  ...
  for (const pegawaiItem of pegawai) {
    ...
    const overlapSql = `
        ...
        AND(
            ...
            LIMIT 1;
`;
    const conflictingSpt = await dbGet(overlapSql, [
      pegawaiId,
      tanggal_berangkat,
      tanggal_kembali,
      tanggal_berangkat,
  ...
  try {
    ...
    const sptPegawaiSql =
      "INSERT INTO spt_pegawai (spt_id, pegawai_id, is_pengikut, urutan) VALUES (?, ?, ?, ?)";
    for (const [index, pegawaiItem] of pegawai.entries()) {
      await runQuery(sptPegawaiSql, [
        newSptId,
        pegawaiItem.id,
        pegawaiItem.pengikut,
        index,
      ]);
    }

    // Buat SPPD otomatis untuk setiap pegawai yang BUKAN pengikut
    const pelaksanaTugas = pegawai.filter(
      (p) => p.pengikut === "0" || p.pengikut === 0,
    );
    ...
    res
      .status(201)
      .json({ message: "SPT dan SPPD berhasil dibuat!", sptId: newSptId });
  } catch (err) {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Data terkait (pejabat, anggaran, atau pegawai) tidak valid. Pastikan pilihan Anda ada di daftar.",
    ...
    res
      .status(500)
      .json({ message: "Gagal menyimpan SPT dan SPPD.", error: err.message });
  }
});
...
nomor_surat = ?, tanggal_surat = ?, dasar_surat = ?, pejabat_pemberi_tugas_id = ?, maksud_perjalanan = ?, lokasi_tujuan = ?, tempat_berangkat = ?, tanggal_berangkat = ?, tanggal_kembali = ?, lama_perjalanan = ?, sumber_dana = ?, kendaraan = ?, anggaran_id = ?, keterangan = ?
    WHERE id = ? `;
      ...
      const laporanTerkait = await dbGet(
        "SELECT id FROM laporan_perjadin WHERE spt_id = ?",
        [id],
      );
      if (laporanTerkait) {
        console.log(
          `[SYNC] Laporan ID ${laporanTerkait.id} ditemukan untuk SPT ID ${id}. Melakukan sinkronisasi...`,
        );
...
dasar_perjalanan = ?,
    ...
    tempat_dikunjungi = ?
        ...
        console.log(
          `[SYNC] Laporan ID ${laporanTerkait.id} berhasil disinkronkan.`,
        );
      }

      await runQuery("COMMIT");
      res.json({
        message: "Surat Perintah Tugas berhasil diperbarui!",
        sptId: id,
      });
    } catch (err) {
      await runQuery("ROLLBACK").catch((rbErr) =>
      ...
      res
        .status(500)
        .json({ message: "Gagal memperbarui SPT.", error: err.message });
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    ...
    const panjarCheckSql =
      "SELECT COUNT(*) as count FROM panjar WHERE spt_id = ?";
    const panjar = await dbGet(panjarCheckSql, [id]);

    if (panjar && panjar.count > 0) {
      return res.status(409).json({
        message:
          "Hapus register/kembalikan uang muka dulu atau perbaiki/edit item Surat Tugas saja.",
      });
    }
    ...
    if (payment && payment.count > 0) {
      return res.status(409).json({
        message:
          "Hapus bukti bayar terlebih dahulu atau perbaiki/edit item Surat Tugas saja.",
      });
    }
    ...
    try {
      ...
      console.log(
        `[DELETE] Mencari dan menghapus laporan terkait untuk SPT ID: ${id} `,
      );
      ...
      await runQuery("ROLLBACK").catch((rbErr) =>
        console.error("[API ERROR] Gagal rollback saat hapus SPT:", rbErr),
      );
      console.error(`[API ERROR] Gagal menghapus SPT id ${id}: `, err);
      res
        .status(500)
        .json({ message: "Gagal menghapus SPT.", error: err.message });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!spt_id || !pegawai_id || !tempat_pembatalan || !tanggal_pembatalan) {
      return res
        .status(400)
        .json({ message: "Data pembatalan tidak lengkap." });
    }
    ...
    try {
      ...
      const insertSql =
        "INSERT INTO pembatalan_spt (spt_id, pegawai_id, tempat_pembatalan, tanggal_pembatalan, alasan, rincian_biaya, nominal_biaya) VALUES (?, ?, ?, ?, ?, ?, ?)";
      ...
      if (totalPegawai > 0 && totalPegawai === totalPembatalan) {
        console.log(
          `[LOGIC] Semua pelaksana di SPT ID ${spt_id} telah dibatalkan.Mengubah status SPT menjadi 'dibatalkan'.`,
        );
        await runQuery("UPDATE spt SET status = 'dibatalkan' WHERE id = ?", [
          spt_id,
        ]);
      }
      ...
      await runQuery("ROLLBACK").catch((rbErr) =>
        console.error("[API ERROR] Gagal rollback pembatalan:", rbErr),
      );
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(
        `[API ERROR] Gagal mengambil pembatalan untuk SPT ID ${spt_id}: `,
        error,
      );
      res.status(500).json({
        message: "Gagal mengambil data pembatalan.",
        error: error.message,
      });
    }
  },
...
router.get("/pembatalan/:id", isApiAuthenticated, async (req, res) => {
  try {
    ...
    console.error(
      `[API ERROR] Gagal mengambil pembatalan id ${req.params.id}: `,
      error,
    );
    res.status(500).json({
      message: "Gagal mengambil data pembatalan.",
    ...
    try {
      ...
      if (result.changes === 0) {
        return res.status(404).json({
          message: "Data pembatalan tidak ditemukan untuk diperbarui.",
        });
      }
      ...
      console.error(
        `[API ERROR] Gagal memperbarui pembatalan id ${id}: `,
        error,
      );
      res
        .status(500)
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const pembatalan = await dbGet(
        "SELECT spt_id FROM pembatalan_spt WHERE id = ?",
        [id],
      );
      if (!pembatalan) {
      ...
      await runQuery("UPDATE spt SET status = 'aktif' WHERE id = ?", [
        pembatalan.spt_id,
      ]);
      ...
      res.json({
        message:
          "Data pembatalan berhasil dihapus dan status SPT telah dikembalikan.",
      });
    } catch (error) {
      await runQuery("ROLLBACK").catch((rbErr) =>
        console.error(
          "[API ERROR] Gagal rollback saat hapus pembatalan:",
          rbErr,
        ),
      );
      ...
      res
        .status(500)
        .json({ message: error.message || "Gagal menghapus data pembatalan." });
    }
  },
);
...
router.get("/cetak/visum/:spt_id", isApiAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (!data) {
      return res
        .status(404)
        .json({ message: "Data SPT atau data terkait tidak ditemukan." });
    }
    res.json(data);
  } catch (error) {
    console.error(
      `[API ERROR] Gagal mengambil data cetak visum untuk SPT ID ${spt_id}: `,
      error,
    );
    res.status(500).json({
      message: "Terjadi kesalahan pada server.",
