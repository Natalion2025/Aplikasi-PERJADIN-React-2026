...
router.get("/search", isApiAuthenticated, async (req, res) => {
  const { q } = req.query;
  if (!q) {
    return res
      .status(400)
  ...
  try {
    ...
    const pegawaiSql = `
        ...
        `;
    const pegawaiResults = await dbAll(pegawaiSql, [
      searchQuery,
      searchQuery,
      searchQuery,
    ]);
    ...
    const formattedResults = [
      ...sptResults.map((item) => ({
        ...
        date: new Date(item.tanggal_surat).toLocaleDateString("id-ID", {
          day: "numeric",
          month: "long",
          year: "numeric",
        }),
