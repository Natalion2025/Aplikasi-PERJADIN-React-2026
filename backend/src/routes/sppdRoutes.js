...
router.post('/api/sppd/auto-create', isApiAuthenticated, async (req, res) => {
    try {
        ...
        // Menggunakan sintaks MySQL YEAR()
        const countSppd = await dbGet("SELECT COUNT(*) as count FROM sppd WHERE YEAR(tanggal_sppd) = ?", [currentYear.toString()]);
        const nomorSppd = `800 / ${countSppd.count + 1} /SETDA/${currentYear}`;

        // Menggunakan MySQL NOW()
        const insertSql = `
