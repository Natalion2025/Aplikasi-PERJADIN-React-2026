...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const sql =
        "SELECT COUNT(*) as count FROM laporan_perjadin WHERE spt_id = ?";
      const result = await dbGet(sql, [spt_id]);
      const exists = result && result.count > 0;
      res.json({ exists });
    } catch (error) {
      console.error(
        `[API ERROR] Gagal memeriksa laporan untuk SPT ID ${spt_id}: `,
        error,
      );
...
router.get(
  ...
  async (req, res) => {
    const { spt_id } = req.params;
    try {
      const laporan = await dbGet(
        "SELECT penandatangan_ids FROM laporan_perjadin WHERE spt_id = ?",
        [spt_id],
      );
      ...
      console.error(
        `[API ERROR] Gagal mengambil penandatangan untuk SPT ID ${spt_id}: `,
        error,
      );
...
router.get("/laporan", isApiAuthenticated, async (req, res) => {
  ...
  try {
    const totalResult = await dbGet(
      "SELECT COUNT(*) as total FROM laporan_perjadin",
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const spt = await dbGet(
        `SELECT id, lokasi_tujuan, tempat_berangkat, lama_perjalanan FROM spt WHERE id = ?`,
        [spt_id],
      );
      if (!spt) {
        return res
      ...
      const cariJenisLokasi = (lokasi) => {
        ...
        for (const group of locationsData) {
          if (group.group.toLowerCase().includes("kecamatan")) {
            for (const location of group.locations) {
              if (
                location.toLowerCase().includes(lokasiLower) ||
                lokasiLower.includes(location.toLowerCase())
              )
                return { jenis: "desa", nama: location, group: group.group };
            }
            if (
              group.group.toLowerCase().includes(lokasiLower) ||
              lokasiLower.includes(
                group.group.toLowerCase().replace("kecamatan", "").trim(),
              )
            )
              return { jenis: "kecamatan", nama: group.group };
          }
      ...
      const cariStandarBiaya = async (tipeBiaya, lokasiQuery) => {
        let query =
          tipeBiaya === "A"
            ? `SELECT * FROM standar_biaya WHERE tipe_biaya = 'A' AND TRIM(UPPER(uraian)) LIKE TRIM(UPPER(?))`
            : `SELECT * FROM standar_biaya WHERE tipe_biaya = 'C' AND TRIM(UPPER(provinsi)) LIKE TRIM(UPPER(?))`;
        let params = [`%${lokasiQuery.trim()}%`];
        if (
          tipeBiaya === "C" &&
          (lokasiQuery.toLowerCase().includes("jakarta") ||
            lokasiQuery.toLowerCase().includes("dki"))
        ) {
          params = ["%JAKARTA%"];
        }
      ...
      if (infoLokasi.jenis === "desa" || infoLokasi.jenis === "kecamatan") {
        ...
        const tempatBerangkat = spt.tempat_berangkat || "Nanga Pinoh";
        const isSameRegion = infoLokasi.nama
          .toLowerCase()
          .includes(tempatBerangkat.toLowerCase());
        standarBiayaHarian = isSameRegion
          ? await cariStandarBiaya("A", infoLokasi.nama)
          : await cariStandarBiaya("C", infoLokasi.provinsi);
        isDalamKota = isSameRegion;
      } else {
        // Provinsi atau tidak diketahui
      ...
      if (!standarBiayaHarian) {
        standarBiayaHarian = await dbGet(
          `SELECT * FROM standar_biaya WHERE tipe_biaya = ? LIMIT 1`,
          [isDalamKota ? "A" : "C"],
        );
      }
      ...
      const representasiInfo = biayaRepresentasiEselonII
        ? {
            uraian: biayaRepresentasiEselonII.uraian,
            harga: isDalamKota
              ? biayaRepresentasiEselonII.biaya_kontribusi || 0
              : biayaRepresentasiEselonII.besaran || 0,
            satuan: biayaRepresentasiEselonII.satuan,
          }
        : null;
      ...
      console.error(
        `[API ERROR] Gagal mengambil rincian pengeluaran untuk SPT ${spt_id}: `,
        error,
      );
      res.status(500).json({
        message: "Terjadi kesalahan pada server saat menghitung rincian.",
      });
    }
  },
);
  ...
  try {
    ...
    ORDER BY sp.urutan ASC
      ...
      const canceledPegawaiSql = `
                ...
                WHERE ps.spt_id = ?
    `;
      laporan.pegawai_dibatalkan = await dbAll(canceledPegawaiSql, [
        laporan.spt_id,
      ]);
    }

    // PERBAIKAN: Hapus query ke tabel 'laporan_pengeluaran' yang sudah tidak ada
    laporan.transportasi = await dbAll(
      "SELECT * FROM laporan_transportasi WHERE laporan_id = ?",
      [req.params.id],
    );
    laporan.akomodasi = await dbAll(
      "SELECT * FROM laporan_akomodasi WHERE laporan_id = ?",
      [req.params.id],
    );
    laporan.kontribusi = await dbAll(
      "SELECT * FROM laporan_kontribusi WHERE laporan_id = ?",
      [req.params.id],
    );
    laporan.lain_lain = await dbAll(
      "SELECT * FROM laporan_lain_lain WHERE laporan_id = ?",
      [req.params.id],
    );
    res.json(laporan);
  } catch (error) {
    console.error(
      `[API ERROR] Gagal mengambil laporan id ${req.params.id}: `,
      error,
    );
    res.status(500).json({ message: "Terjadi kesalahan pada server." });
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      // PERUBAHAN: Logika penyimpanan ke tabel-tabel baru
        ...
        for (const pegawaiId in pegawai) {
            ...
            for (const item of pengeluaran.transportasi) {
              await runQuery(
                ...
                [
                  ...
                  item.nomor_tiket,
                  item.tanggal_tiket || null,
                  item.terminal_berangkat,
                  item.terminal_tiba,
                  item.nominal,
                  item.arah_perjalanan,
                ],
              );
            }
          }
          if (pengeluaran.akomodasi) {
            for (const item of pengeluaran.akomodasi) {
              await runQuery(
                "INSERT INTO laporan_akomodasi (laporan_id, pegawai_id, jenis, nama, harga_satuan, malam, nominal) VALUES (?, ?, ?, ?, ?, ?, ?)",
                [
                  laporanId,
                  pegawaiId,
                  item.jenis,
          ...
          if (pengeluaran.kontribusi) {
            for (const item of pengeluaran.kontribusi) {
              await runQuery(
                "INSERT INTO laporan_kontribusi (laporan_id, pegawai_id, jenis, nominal) VALUES (?, ?, ?, ?)",
                [laporanId, pegawaiId, item.jenis, item.nominal],
              );
            }
          }
          if (pengeluaran.lain_lain) {
            for (const item of pengeluaran.lain_lain) {
              await runQuery(
                "INSERT INTO laporan_lain_lain (laporan_id, pegawai_id, uraian, tarif_satuan, jumlah_hari, nominal, keterangan) VALUES (?, ?, ?, ?, ?, ?, ?)",
                [
                  laporanId,
                  pegawaiId,
                  item.uraian,
      ...
      if (req.files && req.files.length > 0) {
        const lampiranSql =
          "INSERT INTO laporan_lampiran (laporan_id, file_path, file_name, file_type) VALUES (?, ?, ?, ?)";
        for (const file of req.files) {
          const filePath = file.path.replace(/\\/g, "/").replace("public/", "");
          await runQuery(lampiranSql, [
            laporanId,
            filePath,
            file.originalname,
            file.mimetype,
      ...
      res
        .status(201)
        .json({ message: "Laporan berhasil disimpan!", laporanId: laporanId });
    } catch (error) {
      if (req.files && req.files.length > 0) {
      ...
      await runQuery("ROLLBACK").catch((rbErr) =>
...
const recalculateAndUpdatePayment = async (sptId) => {
  console.log(
    `[SYNC - PAYMENT] Memulai sinkronisasi pembayaran untuk SPT ID: ${sptId} `,
  );
  try {
    // 1. Cek apakah ada bukti pembayaran yang terkait dengan SPT ini
    const payment = await dbGet(
      "SELECT id, panjar_data FROM pembayaran WHERE spt_id = ?",
      [sptId],
    );
    if (!payment) {
      console.log(
        `[SYNC - PAYMENT] Tidak ada bukti pembayaran ditemukan untuk SPT ID: ${sptId}. Proses dihentikan.`,
      );
      return;
    }
    ...
    const laporan = await dbGet(
      `SELECT * FROM laporan_perjadin WHERE spt_id = ? `,
      [sptId],
    );
    if (!laporan) {
      console.log(
        `[SYNC - PAYMENT] Tidak ada laporan ditemukan untuk SPT ID: ${sptId}. Tidak dapat menghitung ulang.`,
      );
      return;
    }
    ...
    if (penandatanganIds.length === 0) {
      console.log(
        `[SYNC - PAYMENT] Tidak ada penandatangan di laporan.Nominal diatur ke 0.`,
      );
      await runQuery("UPDATE pembayaran SET nominal_bayar = 0 WHERE id = ?", [
        payment.id,
      ]);
      return;
    }
    ...
    const placeholders = penandatanganIds.map(() => "?").join(",");
    const penerimaFromDb = await dbAll(
      `SELECT id, nama_lengkap, nip, jabatan, golongan FROM pegawai WHERE id IN(${placeholders})`,
      penandatanganIds,
    );

    // 3. Hitung ulang total biaya (mirip logika di /api/laporan/by-spt/:spt_id)
    let grandTotalDibayar = 0;
    const panjarMap = new Map(
      JSON.parse(payment.panjar_data || "[]").map((p) => [
    ...
    for (const penerima of penerimaFromDb) {
      ...
      const totalTransportasi = transportasi
        .filter((t) => t.pegawai_id === penerima.id)
        .reduce((sum, item) => sum + (item.nominal || 0), 0);
      const totalAkomodasi = akomodasi
        .filter((a) => a.pegawai_id === penerima.id)
        .reduce((sum, item) => sum + (item.nominal || 0), 0);
      const totalKontribusi = kontribusi
        .filter((k) => k.pegawai_id === penerima.id)
        .reduce((sum, item) => sum + (item.nominal || 0), 0);
      const totalLainLain = lainLain
        .filter((l) => l.pegawai_id === penerima.id)
        .reduce((sum, item) => sum + (item.nominal || 0), 0);

      totalBiayaPegawai =
        totalTransportasi + totalAkomodasi + totalKontribusi + totalLainLain;
    ...
    console.log(
      `[SYNC - PAYMENT] Nominal baru dihitung: ${grandTotalDibayar}. Memperbarui pembayaran ID: ${payment.id} `,
    );
    await runQuery("UPDATE pembayaran SET nominal_bayar = ? WHERE id = ?", [
      grandTotalDibayar,
      payment.id,
    ]);

    console.log(
      `[SYNC - PAYMENT] Sinkronisasi pembayaran untuk SPT ID: ${sptId} berhasil.`,
    );
  } catch (error) {
    console.error(
      `[SYNC - PAYMENT - ERROR] Gagal menyinkronkan pembayaran untuk SPT ID ${sptId}: `,
      error,
    );
...
router.put(
  ...
  async (req, res) => {
    ...
    const deletedFiles = data.deleted_files
      ? JSON.parse(data.deleted_files)
      : [];
    let sptIdForSync = null; // Variabel untuk menyimpan spt_id

    try {
      // PERUBAHAN: Logika update ke tabel-tabel baru
      await runQuery("BEGIN TRANSACTION");
      ...
      const sptIdFromForm =
        data.spt_id ||
        (await dbGet("SELECT spt_id FROM laporan_perjadin WHERE id = ?", [id]))
          ?.spt_id;
      if (sptIdFromForm) {
        const paymentCheck = await dbGet(
          "SELECT COUNT(*) as count FROM pembayaran WHERE spt_id = ?",
          [sptIdFromForm],
        );
      ...
      if (deletedFiles.length > 0) {
        ...
        const filesToDelete = await dbAll(
          `SELECT file_path FROM laporan_lampiran WHERE id IN(${placeholders})`,
          deletedFiles,
        );
        ...
        await runQuery(
          `DELETE FROM laporan_lampiran WHERE id IN(${placeholders})`,
          deletedFiles,
        );
      }

      // Ambil spt_id sebelum update, untuk sinkronisasi nanti
      const oldLaporan = await dbGet(
        "SELECT spt_id FROM laporan_perjadin WHERE id = ?",
        [id],
      );
      if (oldLaporan) {
      ...
      await runQuery(
        `UPDATE laporan_perjadin SET ${updateFields.join(", ")} WHERE id = ? `,
        updateValues,
      );

      // 3. Hapus semua data pengeluaran lama dan masukkan yang baru
      await runQuery("DELETE FROM laporan_transportasi WHERE laporan_id = ?", [
        id,
      ]);
      await runQuery("DELETE FROM laporan_akomodasi WHERE laporan_id = ?", [
        id,
      ]);
      await runQuery("DELETE FROM laporan_kontribusi WHERE laporan_id = ?", [
        id,
      ]);
      await runQuery("DELETE FROM laporan_lain_lain WHERE laporan_id = ?", [
        id,
      ]);
      ...
      if (pegawai) {
        for (const pegawaiId in pegawai) {
          ...
          if (pengeluaran.akomodasi) {
            for (const item of pengeluaran.akomodasi) {
              await runQuery(
                "INSERT INTO laporan_akomodasi (laporan_id, pegawai_id, jenis, nama, lokasi_hotel, tanggal_checkIn, tanggal_checkOut, harga_satuan, malam, nominal) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [
                  id,
                  pegawaiId,
                  item.jenis,
                  item.nama,
                  item.lokasi_hotel,
                  item.tanggal_checkIn || null,
                  item.tanggal_checkOut || null,
          ...
          if (pengeluaran.kontribusi) {
            for (const item of pengeluaran.kontribusi) {
              await runQuery(
                "INSERT INTO laporan_kontribusi (laporan_id, pegawai_id, jenis, nominal) VALUES (?, ?, ?, ?)",
                [id, pegawaiId, item.jenis, parseCurrency(item.nominal)],
              );
            }
          }
          if (pengeluaran.lain_lain) {
...
router.delete("/laporan/:id", isApiAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const lampiran = await dbAll(
      "SELECT file_path FROM laporan_lampiran WHERE laporan_id = ?",
      [id],
    );

    // Hapus record laporan (akan menghapus lampiran via CASCADE)
    const result = await runQuery("DELETE FROM laporan_perjadin WHERE id = ?", [
      id,
    ]);
    if (result.changes === 0) {
      return res.status(404).json({ message: "Laporan tidak ditemukan." });
