...
// POST new anggaran
router.post('/api/anggaran', isApiAuthenticated, isApiAdminOrSuperAdmin, async (req, res) => {
    const { bidang_urusan, program, kegiatan, sub_kegiatan, mata_anggaran_kode, mata_anggaran_nama, nilai_anggaran, pptk_id } = req.body;
    ...
    if (!kode || !nilai_anggaran) {
        return res.status(400).json({ message: 'Mata Anggaran Kode dan Nilai Anggaran wajib diisi.' });
    }
...
// PUT update anggaran
router.put('/api/anggaran/:id', isApiAuthenticated, isApiAdminOrSuperAdmin, async (req, res) => {
    const { id } = req.params;
    const { bidang_urusan, program, kegiatan, sub_kegiatan, mata_anggaran_kode, mata_anggaran_nama, nilai_anggaran, pptk_id } = req.body;
...
// DELETE anggaran
router.delete('/api/anggaran/:id', isApiAuthenticated, isApiAdminOrSuperAdmin, async (req, res) => {
    const { id } = req.params;
    try {
