...
p.id,
    ...
    (SELECT SUM(pr.jumlah) FROM panjar_rincian pr WHERE pr.panjar_id = p.id) as total_biaya
            ...
            LEFT JOIN pegawai pelaksana ON p.pelaksana_id = pelaksana.id
...
router.get("/api/panjar/:id", isApiAuthenticated, async (req, res) => {
  try {
    ...
    if (!panjar) {
      return res
        .status(404)
        .json({ message: "Data uang muka tidak ditemukan." });
    }
    panjar.rincian = await dbAll(
      "SELECT * FROM panjar_rincian WHERE panjar_id = ?",
      [req.params.id],
    );
    res.json(panjar);
  } catch (error) {
    console.error(
      `[API ERROR] Gagal mengambil panjar id ${req.params.id}: `,
      error,
    );
    res.status(500).json({ message: "Terjadi kesalahan pada server." });
  }
...
router.post("/api/panjar", isApiAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const panjarResult = await runQuery(panjarSql, [
      tempat,
      tanggal_panjar,
      spt_id,
      bendahara_id,
    ...
    res
      .status(500)
      .json({ message: "Gagal menyimpan data uang muka.", error: err.message });
  }
});
...
router.delete("/api/panjar/:id", isApiAuthenticated, async (req, res) => {
  try {
    ...
    console.error(
      `[API ERROR] Gagal menghapus panjar id ${req.params.id}: `,
      err,
    );
    res
      .status(500)
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const panjar = await dbGet(
        `SELECT id FROM panjar WHERE spt_id = ? AND pelaksana_id = ? `,
        [spt_id, pegawai_id],
      );
      if (!panjar) {
        return res
          .status(404)
          .json({ message: "Tidak ada data uang muka untuk pegawai ini." });
      }

      const rincianItems = await dbAll(
        `SELECT uraian, jumlah FROM panjar_rincian WHERE panjar_id = ? `,
        [panjar.id],
      );
      if (rincianItems.length === 0) {
        return res
          .status(404)
          .json({ message: "Rincian uang muka tidak ditemukan." });
      }
      ...
      console.error(
        `[API ERROR] Gagal mengambil data panjar untuk SPT ${spt_id} & Pegawai ${pegawai_id}: `,
        error,
      );
      res.status(500).json({ message: "Terjadi kesalahan pada server." });
    }
...
router.get("/api/cetak/panjar/:id", isApiAuthenticated, async (req, res) => {
  try {
    ...
    console.error(
      `[API ERROR] Gagal mengambil data cetak panjar id ${req.params.id}: `,
      error,
    );
    res.status(500).json({ message: "Terjadi kesalahan pada server." });
  }
